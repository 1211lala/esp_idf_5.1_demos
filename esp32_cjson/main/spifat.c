...
{
    ESP_LOGI("flashfat_mount", "flashfat mount fatfs");
    ESP_ERROR_CHECK(esp_vfs_fat_spiflash_mount_rw_wl(fat->root_path, fat->partition_label, &fat->mount_config, &fat->wl_handle));
    list_files(fat->root_path);
}

void spifat_unmount(struct FLASHFAT *fat)
{
    ESP_LOGE("spifat_unmount", "flashfat unmount fatfs");
